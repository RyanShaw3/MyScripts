...
class HostlocGetPoints:
    ...
    def check_login_status(self, s: req_Session, number_c: int) -> bool:
        ...
        if len(test_title) != 0:  # 确保正则匹配到了内容，防止出现数组索引越界的情况
            if test_title[0] != "个人资料 -  全球主机交流论坛 -  Powered by Discuz!":
                ...
                self.text = self.text + "当前账户：" + name + "\n"
                print("当前账户：" + name)
                return True
        else:
            self.text = self.text + "无法在用户设置页面找到标题，该页面存在错误或被防 CC 机制拦截！\n"
    ...
    def print_current_points(self, s: req_Session):
        ...
        if len(points) != 0:  # 确保正则匹配到了内容，防止出现数组索引越界的情况
            self.text = self.text + "帐户当前积分：" + points[0] + "\n"
            print("帐户当前积分：" + points[0])
        else:
            self.text = self.text + "无法获取帐户积分，可能页面存在错误或者未登录！" + "\n"
        ...
        if self.check_login_status(s, number_c):
            ...
            for i in range(len(url_list)):
                ...
                try:
                    ...
                    print("链接访问异常：" + str(e))
                continue
            self.text = self.text + "用户空间成功访问{}个，访问失败{}个\n".format(success, fail)
            self.print_current_points(s)  # 再次打印帐户当前积分
        else:
            self.text = self.text + "请检查你的帐户是否正确！\n"
            print("请检查你的帐户是否正确！")
    ...
    def print_my_ip(self):
        ...
        try:
            ...
            res.raise_for_status()
            res.encoding = "utf-8"
            self.text = self.text + "当前使用 ip 地址：" + res.text.replace(".", ",") + "\n"
            print("当前使用 ip 地址：" + res.text)
        except Exception as e:
            self.text = self.text + "获取当前 ip 地址失败：" + str(e) + "\n"
            print("获取当前 ip 地址失败：" + str(e))

    def server_send(self, sendkey):
        if sendkey == "":
            return
        server_url = "https://sc.ftqq.com/" + str(sendkey) + ".send"
        data = {"text": "hostloc刷分成功", "desp": self.text}
        requests.post(server_url, data=data)

    def hostloc_get_points(self, usernames, passwords, sendkey):
        # 分割用户名和密码为列表
        user_list = usernames.split(",")
        passwd_list = passwords.split(",")
        if not usernames or not passwords:
            self.text = self.text + "未检测到用户名或密码，请检查环境变量是否设置正确！\n"
            print("未检测到用户名或密码，请检查环境变量是否设置正确！")
        elif len(user_list) != len(passwd_list):
            self.text = self.text + "用户名与密码个数不匹配，请检查环境变量设置是否错漏！\n"
            print("用户名与密码个数不匹配，请检查环境变量设置是否错漏！")
        else:
            self.print_my_ip()
            self.text = self.text + "共检测到{}个帐户，开始获取积分\n".format(len(user_list))
        ...
        print(self.text)
        self.server_send(sendkey)


def main():
    usernames = os.environ["HOSTLOC_USERNAME"]
    passwords = os.environ["HOSTLOC_PASSWORD"]
    sendKey = os.environ["SERVER_SENDKEY"]
    h = HostlocGetPoints()
    h.hostloc_get_points(usernames, passwords, sendKey)
